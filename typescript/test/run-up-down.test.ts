...
const writePair = async (dir: string, name: string): Promise<MigrationDescriptor> => {
...
describe('runUp / runDown against a mocked client', () => {
  it('applies the first pending migration and records checksum + log success', async () => {
    ...
    expect(client.applied.get('0001_init')).toBe(checksum(await readFile(first.upPath, 'utf8')));
    expect(client.runs.some((r) => /INSERT INTO\s+"?migrates"?/i.test(r.sql))).toBe(true);
    expect(
      client.runs.some(
  ...
  it('records an error log and rethrows when exec fails inside the transaction', async () => {
    ...
    await expect(runUp({ client, migrations: [first], env: {} })).rejects.toThrow('ddl failed');
    expect(
      client.runs.some((r) => /UPDATE\s+"?migrate_logs"?/i.test(r.sql) && r.params[0] === 'error'),
    ).toBe(true);
  });
});
